...
def _geometry_region(context, region):
    ...
    for kind in (1, 2, 3):
        ...
        for fold in range(fold_count):
            ...
            # La distancia entre muestras no depende del modelo candidato.
            if fold not in fold_nearest:
                fold_nearest[fold] = cKDTree(q[train]).query(q[test])[0]
            nearest = fold_nearest[fold]
            covered &= nearest <= 3 * h
            perpose = []
