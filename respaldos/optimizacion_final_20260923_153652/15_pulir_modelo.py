...
def contextual_surface_model(q, normals, confidence, mid, h):
    ...
    directions = []
    confidences = []
    # Agrupar una sola vez en vez de recorrer todos los puntos por celda.
    # La ordenación estable conserva el orden de muestras de flatnonzero.
    ...
    normal_cosine = np.cos(np.deg2rad(40))
    for group in range(len(group_counts)):
        ids = group_order[group_starts[group]:group_ends[group]]
        nn = normals[ids]
        ...
        if np.mean(nn @ direction < normal_cosine) > 0.25:
            continue
