...
def resolve_local_layers_and_normals(points, normals, confidence, support, spacing):
    ...
    tree = cKDTree(points)
    # Preparar vecinos del siguiente bloque mientras se evalúa el actual.
    # points y tree no se modifican; las decisiones mantienen el orden original.
    ...
    def query_block(start):
        ...
        return start, stop, distances, neighbors

    normal_family_cosine = np.cos(np.deg2rad(30.0))
    cartesian_axes = np.eye(3)
    for start, stop, distances, neighbors in prefetch_items(
        query_block, range(0, len(points), 1024)
    ):
        for row, i in enumerate(range(start, stop)):
            ...
            dot = n[ix] @ n[i]
            family = dot >= normal_family_cosine
            ...
            if len(ids) < 12:
                continue
            axis = cartesian_axes[np.argmin(np.abs(n[i]))]
            a = np.cross(n[i], axis)
            a /= np.linalg.norm(a)
            b = np.cross(n[i], a)
